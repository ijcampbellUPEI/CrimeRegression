...
We needed observations of the amount of crime being committed in communities and the size of their police forces.  We downloaded data from Stats Canada with a crime severity index (CSI) for municipalities across Canada.  The index is a measurement of crime rates with more serious crimes being weighed higher in the calculation.  We filtered the data to only use observations from 2021.

The full dataset can be accessed here[https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=3510007601].
...
We converted population size into four categories of roughly equal size.
We removed 2 observation of towns with 0 police officers, and one extreme outlier.  Manawan, Quebec has a crime severity index an order of magnitude higher than the next highest, so we have removed it.  We also shortened some of the column headers.
...
```{r}
policing <- policeData %>% select(dep, crime_severity_index, officers_per_100000, population_level, authorized_officers_per_100000)
```
...
_Address missing values._ We see that there are no missing values in our data.
...
We can see that crime severity index is most correlated with police officers per 100,000 in smaller towns.  They also seem to have the highest mean and median crime severity index.  CSI appears to be correlated with officers per 100,000 residents, but the standard deviations seem high and analysis of variance will be important.

## Histogram of outcome variable 

_Visualize the distribution of the outcome variable using a histogram and comment._

```{r, fig.cap = "Figure 1. Crime Severity Index in Canadian Municipalities", fig.align = "center", fig.width=16/2, fig.height=9/2}
...
CSI distribution bell shaped, but right-skewed, which is not unexpected because it is ideally small and has a minimum of 0.  This could be signaling that there is not constant variance and that the data will require a transformation.
...
```{r, fig.cap = "Figure 2. CSI vs Police Officers per 100,000", fig.align = "center", fig.width=16/2, fig.height=9/2}
...
```
There appear to be a positive correlation between police officers per 100,000 and crime severity index.  We had wondered if higher relative police strength would be negatively correlated with CSI, but that does not what we see.  There does seem to be fanning out.
...
```{r, fig.cap = "CSI Distribution by Population Level", fig.align = "center", fig.width=16/2, fig.height=9/2}
ggplot(policing, aes(x = crime_severity_index, y = population_level, fill=population_level )) +
  geom_boxplot() +
  labs(x = "Crime Severity Index", y="Population Level")
```

The smallest municipalities tend to have higher mean crime severity.  The CSI of higher population centers are more normally distributed, but there are outliers at all levels.
...
```{r, fig.cap = "Figure 4. CSI ~ Officers per 100,000", fig.align = "center", fig.width=16/2, fig.height=9/2}
ggplot(policing, aes(x=officers_per_100000, y=crime_severity_index, color=population_level)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE ) +
  labs(y = "Crime Severity Index", x = "Police Officers per 100,000")
```
The colored scatterplot allows us to visualize the difference in correlation across population levels.  We see that there is a point of high leverage for the 6000-63,000 and 13,000-63,000 levels, which are part of the reason we see a smaller slope for those lines.  The line for the >63,000 level is difficult to see because it is not extrapolated out to the length of the others, but appears to have the highest slope.
